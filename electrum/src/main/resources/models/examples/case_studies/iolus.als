...
abstract sig KDS {
  ...
  Monotonic[keys]
  all t : Tick | let t1 = ord/prev[t] {
    all m : members[t]-members[t1] | Join[m, t, this]
    all m : members[t1]-members[t] | Leave[m, t]
  }
}
...
{ SendMessage[sender, sentTime, this] ||
  (some msg1 : DataMessage |
     Remulticast[gsaID, msg1, retransmitTime, this]) }
...
fact IolusProperties {
  no k, k1 : GroupKey | k!=k1 && k.generator = k1.generator && k.generatedTime = k1.generatedTime
  all g : GSA, msg : DataMessage, t : Tick | RemulticastConditions[g, msg, t] =>
    (some msg1: DataMessage | Remulticast[g, msg, t, msg1])
}
...
fact {
  ...
  no m, m1 : DataMessage {
    m!=m1
    m.sender = m1.sender
    m.sentTime = m1.sentTime
    m.key = m1.key
    m.gsaID = m1.gsaID
    m.retransmitTime = m1.retransmitTime }
}
...
pred SendRequest[c : Client, gsa : GSA, t : Tick, msg : DataMessage] {
  ...
  (some gsa.parent.members[t]) =>
    (some msg1 : DataMessage | Remulticast[gsa, msg, t, msg1]) }
...
pred CanReceive[m : Member, t : Tick, msg : Message] {
  some msg1 : DataMessage {
    msg1.sentTime = msg.sentTime
    msg1.sender = msg.sender
    msg1 in m.receivedMessages[ord/prev[t]] || ReceiveConditions[m, t, msg1] }}
...
pred Remulticast[g : GSA, msg : DataMessage, t : Tick, msg1: lone DataMessage] {
  RemulticastConditions[g, msg, t]
  let g1 = g.parent + g - msg.gsaID | NewestKey[g1.keys[msg.sentTime], msg1.key]
  msg1.sender = msg.sender
  msg1.sentTime = msg.sentTime
  msg1.retransmitTime = t
  msg1.gsaID = g
}
...
assert Connected {
  all g, g1 : GSA | g in g1.*(parent + ~parent) }
...
pred LoopFree {
  no t, t1 : Tick {
    t!=t1
    all k : KDS | k.members[t] = k.members[t1] -- no constraint on keys
    all m : Member | m.receivedMessages[t] = m.receivedMessages[t1]
    all m : DataMessage | m.retransmitTime = t =>
      (some m1 : DataMessage {
        m1.retransmitTime = t1
        m.sender = m1.sender
        m.sentTime = m1.sentTime
        m.gsaID = m1.gsaID
        m.key = m1.key })
...
assert NonLinearTopology {
  (no g : GSA | #g.~parent > 1) ||
  !(some m, m1 : DataMessage | Remulticast[m.gsaID, m1, m.retransmitTime, m]
...
assert x {
  !(LoopFree && some DataMessage &&
     (some t : Tick | some m, m1 : Member |
        m!=m1 && IsMember[m, t] && IsMember[m1, t] && t != ord/next[ord/first]))
}
...
assert InsiderCanRead {
  all msg : Message, m : Member |
    some t : Tick - ord/last | all t1 : ord/nexts[t] |
      (IsMember[msg.sender, msg.sentTime] &&
       IsMember[m, msg.sentTime]) => CanReceive[m, t1, msg]
}
